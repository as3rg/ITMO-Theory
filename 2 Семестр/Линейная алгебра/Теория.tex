...
$\nu_1, \ldots, \nu_m$ -- базис $V$\\
$\fall u \in U\ u = \suml_{i = 1}^n u_i \xi_i \lrto u = \begin{pmatrix}
u_1\\u_2\\\vdots\\u_n
\end{pmatrix}$\\
$\fall v \in V\ v = \suml_{i = 1}^m v_i \nu_i \lrto v = \begin{pmatrix}
v_1\\v_2\\\vdots\\v_m
\end{pmatrix}$\\
$\fall v \in \im \mcA\ v = \mcA u = \suml_{i=1}^n u_i \mcA \xi_i$\\
$\mcA$, как линейное отображение, полностью определяется значениями $\mcA$ на базисных векторах\\
$\mcA_{\xi_i} = \suml_{j=1}^m a_{ji} \nu_j \lrto \begin{pmatrix}
    a_{1i}\\\vdots\\a_{mi}
\end{pmatrix}$\\
...
$v = \suml_{j=1}^m v_j \nu_j$\\
$v = \mcA u = \suml_{i = 1}^n u_i \mcA_{\xi_i} = \suml_{i=1}^n u_i\suml_{j=1}^m a_{ji}\nu_j = \suml_{j=1}^m(\suml_{i=1}^n a_{ji}u_i)\nu_j$\\
Т.к. координаты введены единственным образом, то $\fall j\ v_j = \suml_{i=1}^n a_{ji}u_i \LRto v = \mcA u \LRto v = Au$\\
\textbf{Примеры}
\begin{enumerate}
...
Докажем линейность\\
$(\mcA+\lambda \mcB)_{\xi_i} = \mcA_{\xi_i}+\lambda\mcB_{\xi_i} = \suml_{j=1}^m a_{ji}\nu_j + \lambda \suml_{j=1}^m b_{ji}\nu_j = \suml_{j=1}^m(a_{ji} + \lambda b_{ji})\nu_j \lrto A+\lambda B$\\\\
\textbf{Утверждение}\\
$\mcA\mcB \lrto AB$\\
\textbf{Доказательство}\\
$\underset{\xi_1\ldots\xi_n}{U}\xrto[\mcB]{} \underset{\theta_1\ldots\theta_r}{W} \xrto[\mcA]{} \underset{\nu_1\ldots\nu_m}{V}$
$(\mcA\mcB)_{\xi_i} = \mcA(\mcB_{\xi_i}) = \mcA(\suml_{k=1}^r b_{ki}\theta_k) = \suml_{k=1}^r b_{ki}\mcA_{\theta_k} = \suml_{k=1}^rb_{ki}\suml_{j=1}^m a_{jk}\nu_j = \suml_{j=1}^m(\suml_{k=1}^r a_{jk}b_{ki})\nu_i = \suml_{j=1}^m AB_{jk} \nu_j \lrto \begin{pmatrix}
    \vdots
\end{pmatrix}$\\
\textbf{Утверждение}\\
...
$\det \mcA = \det (\mcA e_1, \ldots, \mcA e_n) = \det(\suml_{i_1=1}^n a_{i_11}e_{i_1}, \ldots, \suml_{i_n=1}^n a_{i_nn}e_{i_n}) =\\ \suml_{i_1=1}^n\ldots\suml_{i_n=1}^n a_{i_11}\ldots a_{i_nn}\det(e_{i_1}, \ldots, e_{i_n}) = \suml_{\sigma \in S_n}(-1)^{\nm{inv}(\sigma)}a_{i_11}\ldots a_{i_nn}\det(e_1,\ldots, e_n) = \suml_{\sigma \in S_n}(-1)^{\nm{inv}(\sigma)}a_{i_11}\ldots a_{i_nn} = \det A$\\
...
\textbf{Определение}\\
$A_{n\times n}$\\
$\tr A := \suml_{i=1}^n a_{ii}$ -- след матрицы\\
\textbf{Теорема 3}\\
Если матрицы подобные, то $\tr A = \tr B$\\
\textbf{Доказательство}\\
$A, B$ -- подобные $\Rto \ex$ невырожденная $C: A=C^{-1}BC=SBC$\\
$\tr A = \suml_{i=1}^na_{ii}=\suml_{i=1}^n\suml_{k=1}^n S_{ik}(BC)_{ki} = \suml_{i=1}^n\suml_{k=1}^n S_{ik}\suml_{m=1}^nB_{km}C_{ki} = \suml_{m=1}^n\suml_{k=1}^n B_{km}\suml_{i=1}^nC_{mi}S_{ik} = \suml_{m=1}^n\suml_{k=1}^n B_{km} E_{mk} = \suml_{k=1}^n B_{kk} = \tr B$\\
\textbf{Следствие}\\
...
$\mcA e_{j\in 1\ldots k} \in L \Rto \mcA e_j = \suml_{i=1}^k a_{ij}e_i \lrto A_j = \begin{pmatrix}
    a_{1j}\\
    \vdots\\
    a_{k_j}\\
    0\\
...
$L_i \subset V$ -- инвариантные линейные пространства относительно $\mcA \in \nm{End}(V)$\\
$\mcA_i = \mcA: L_i \rto V$\\
Тогда $\im \mcA = \oplusl_{i=1}^m \im \mcA^j$\\
\textbf{Доказательство}\\
$V = \oplusl_{i=1}^m L_i$\\
$\fall x\in V\ \ex! x_1 \in L_1,\ldots,x_m \in L_m:\ x=\suml_{i=1}^m x_i$\\
$\mcA x = \suml_{i=1}^m \mcA x_i$\\
$\mcA x_i \in \im \mcA_i$\\
Отсюда $\im \mcA = \suml_{i=1}^m \im \mcA_i$\\
...
\begin{enumerate}
    ...
    Тогда $v_{m+1} = \suml_{i=1}^m \alpha_i v_i$\\
    //todo
\end{enumerate}
\textbf{Следствие 1}\\
$\lambda_1, \ldots, \lambda_m$ попарно различные СЧ $\mcA$\\
...
\textbf{Теорема}\\
Если все корни $\chi_\mcA(t) \in K$, т.е. являются СЧ (т.е. $\suml_{\lambda \text{ -- СЧ}} \alpha(\lambda) = n = \deg \chi_\mcA(t)$)\\
$\mcA$ -- ОПС $\LRto \fall \lambda$ -- СЧ $\alpha(\lambda) = \gamma(\lambda)$\\
\textbf{Доказательство}\\
$\gamma(\lambda) \leq \alpha(\lambda)$\\
$\mcA$ -- ОПС $\LRto \ex$ базис из СВ $\LRto М = \oplusl_{\lambda \text{ -- СЧ}} V_{\lambda} \LRto n = \suml_{\lambda \text{ -- СЧ}} \gamma(\lambda)$\\
Отсюда $n = \suml_{\lambda \text{ -- СЧ}} \alpha(\lambda), \alpha = \gamma$\\
\textbf{Следствие}\\
...
//todo\\
\textbf{Определение}\\
Пусть $V = \oplusl_{i=1}^m L_i, L_i$ -- линейное подпространство\\
Тогда $\fall v\in V \ex! v_1, \ldots v_m: v_i \in L_i, v=\suml_{i=1}^m v_i$\\
Зададим $\rho_i \in \nm{End}(V): \rho_i v = v_i \in L_i$\\
$\rho_i$ -- \textit{оператор проектирования(проектор)} на $L_i$\\
Свойства:
\begin{enumerate}
    \item $\fall i \neq j\ \rho_i \rho_j = \0$
    \item $\suml_{i=1}^m \rho_i = \epsilon$
...
Тогда $V = \oplusl_{i=1}^m \im \rho_i$ (т.е. $\rho$ -- проектор на $L_i = \im \rho_i$)\\
\textbf{Доказательство}\\
Докажем $1, 2 \Rto 3$\\
$\rho_i = \rho_i \epsilon = \rho_i \suml_{i=1}^m \rho_j = \rho_i^2$\\
Докажем, что $V = \oplusl_{i=1}^m \im \rho_i$\\
$\fall v \in V\ v = \epsilon v = \suml_{i=1}^m \rho_i v \Rto V = \suml_{i=1}^m \im \rho_i$\\
Докажем дизъюнктность\\
$\0 = v_1 (\in \im \rho_1) + \ldots + v_m (\in \im \rho_m)$\\
$\fall i = 1\ldots m\ v_i = \rho_i \omega_i (\ex \omega_i \in V)$\\
$v_i = \rho_i \omega_i = $ из свойства 3 $ = \rho_i (\suml_{j=1}^m \rho_j \omega_j) = \rho_i (\suml_{j=1}^m v_j) = \rho_i \0 = \0$, ч.т.д.\\
\textbf{Теорема о спектральном разложении о.п.с.}\\
$\fall \mcA \in \nm{End}(V)$ -- о.п.с.\\
Тогда $\mcA = \suml_{\lambda \text{ -- С.Ч.}} \lambda \rho_\lambda$, где $\rho_\lambda$ -- проектор на $V_\lambda$\\
\textbf{Доказательство}\\
Обозначение: Пусть все $\lambda$ -- С.Ч.
$\mcA$ -- о.п.с $\LRto V = \oplusl_{\lambda} V_\lambda$\\
$v = \suml_{\lambda} v_\lambda, v_\lambda \in V_\lambda$\\
$\fall v\in V\ \mcA v = \mcA(\suml_\lambda v_\lambda) = \suml_\lambda (\mcA v) = \suml_\lambda \lambda v_\lambda = \suml_\lambda \lambda\rho_\lambda(v) = (\suml_\lambda \lambda\rho_\lambda) v$\\
Отсюда $\mcA = \suml_\lambda \lambda \rho_\lambda$ -- \textit{спектральное разложение}\\
\textbf{Следствие}\\
$A$ -- диагонализируема $\Rto\ex \rho_\lambda, \lambda$ -- С.Ч. $A: A = \suml_\lambda \lambda \rho_\lambda$\\
\textbf{Определение}\\
$(A_m)_{m=1}^\infty$ -- последовательность матриц $A_{n\times n} = (a_{ij}^m)_{n\times n}, m$ -- индекс, а не степень\\
$\liml_{m \rto \infty} A_m = A = (a_{ij})_{n\times n} \LRto \fall i,j = 1\ldots n\ a_{ij}=\liml_{m\rto \infty} a_{ij}^m$\\
\textbf{Немного про ряды}\\
$\suml_{m=1}^\infty a_m$ -- числовой ряд ($a_m \in \Rset(\Cset)$)\\
$S_m = \suml_{i=1}^m a_k$ -- частичная сумма ряда\\
Если $S_m$ сходится, то ряд называется сходящимся\\
\textbf{Определение}\\
$\suml_{m=1}^\infty A_m$ -- ряд из матриц\\
$\suml_{m=1}^\infty A_m$ -- сходится $\LRto \fall i,j = 1\ldots n\ \suml_{m=1}^\infty a_{ij}^m$ -- сходится\\
\textbf{Далее про ряды}\\
$\suml_{n=1}^\infty u_n(x), u_n: \Rset \rto \Rset$ -- функциональный ряд\\
При фиксированном $x$ -- числовой ряд\\
Множество $x$ таких, что числовой ряд сходится -- множество поточечной сходимости ряда $= E$\\
$\suml_{m=1}^\infty C_m (x-x_0)^m$ -- степенные ряды\\
Утверждается, что ряд сходится при $|x-x_0| < R$, где $R$ -- радиус сходимости\\
В $\Cset$ -- круг сходимости\\
В $\Rset$ -- интервал сходимости\\
Для $\Rset:\ \frac1R = \overline{\liml_{m\rto \infty}} \sqrt[m]{|C_m|}$\\
Примеры сходящихся рядов -- ряды Тейлора-Маклорена\\
$e^x = \suml_{m=1}^\infty \frac{x^m}{m!}$, сходится при $|x-x_0| \leq \infty$\\
$\sin x = \suml_{m=0}^\infty \frac{(-1)^m x^{2m+1}}{(2m+1)!}$, сходится при $|x| \leq \infty$\\
На окружности (при $|x-x_0| = R$) ряд может как сходиться, так и расходиться\\
\textbf{Определение}\\
Пусть $f(x) = \suml_{m=0}^\infty C_m x^m, |x| \leq R$\\
Тогда $f(A):= \suml_{m=0}^\infty C_m A^m$ (если ряд сходится)\\
\textbf{Теорема 1 (первый способ вычисления $f(A)$ для диагонализируемой матрицы)}\\
Пусть $A_{n\times n}$ диагонализируема\\
$f(x) = \suml_{m=1}^\infty C_m x^m, |x| \leq R$\\
Тогда если $\fall \lambda$ -- СЧ $|\lambda| < R$, то ряд $\suml_{m=1}^\infty C_m A^m$ сходится\\
$f(A) = T\nm{diag}(f(\lambda_1), \ldots, f(\lambda_n))T^{-1}$, где $\Lambda = T^{-1}AT = \nm{diag}(\lambda_1, \ldots, \lambda_n)$\\
\textbf{Теорема}\\
$A_{n\times n}$ диагонализируема, а значит $\ex T: \Lambda = T^{-1}AT = \nm{diag}(\lambda_1, \ldots, \lambda_n)$\\
$S_m = \suml_{k = 0}^m C_k A^k, R = \infty$\\
$A^k = (T \Lambda T^{-1})^k = T \Lambda^k T^{-1} = T\nm{diag}(\lambda_1^k, \ldots, \lambda_n^k)T^{-1}$\\
Отсюда $S_m = T\nm{diag}(\suml_{k=0}^m C_k\lambda_1^k, \ldots, \suml_{k=0}^m C_k\lambda_n^k)T^{-1}$ (т.к. $R = \infty$, то все ряды сойдутся)\\
$S = \liml_{m\rto \infty} S_m = T\nm{diag}(f(\lambda_1), \ldots, f(\lambda_n))T^{-1}$\\
\textbf{Теорема 2 (второй способ вычисления $f(A)$ для диагонализируемой матрицы)}\\
Пусть $A_{n\times n}$ диагонализируема\\
$f(x) = \suml_{m=0}^\infty C_m x^m, |x| \leq R$\\
Тогда если $\fall \lambda$ -- СЧ $|\lambda| < R$, то ряд $\suml_{m=1}^\infty C_m A^m$ сходится\\
$f(A) = \suml_{\lambda \text{-- СЧ}} f(\lambda) \rho_\lambda$, где $A = \suml_{\lambda \text{ -- СЧ}} \lambda\rho_\lambda$ -- спектральное разложение\\
\textbf{Доказательство}\\
$A$ -- диагонализируема $\Rto A = \suml_{\lambda \text{ -- СЧ}} \lambda \rho_\lambda$\\
Тогда $A^k = (\suml_{\lambda \text{ -- СЧ}} \lambda \rho_\lambda)^k = \suml_{\lambda \text{ -- СЧ}} \lambda^k \rho_\lambda$\\
Отсюда $S_m = \suml_{k=0}^m C_k A^k = \suml_{k=0}^m C_k\suml_{\lambda \text{ -- СЧ}} \lambda^k \rho_\lambda =  \suml_{\lambda \text{ -- СЧ}} (\suml_{k=0}^mC_k\lambda^k) \rho_\lambda \xrto[m \rto \infty]{} \suml_{\lambda \text{ -- СЧ}} f(\lambda) \rho_\lambda$\\
\textbf{Следствие}\\
$A$ -- диагонализируема, $f(x) = \suml_{m=0}^\infty C_m x^m, |x| < R$\\
$\fall \lambda$ -- СЧ $|\lambda| < R$\\
$t \in \Rset(\Cset): \fall \lambda$ -- СЧ $|t\lambda| < R$\\
Тогда $f(At) = T\nm{diag}(f(\lambda_1 t), \ldots, f(\lambda_nt))T^{-1}$\\
или $f(At) = \suml_{\lambda \text{-- СЧ}} f(\lambda t) \rho_\lambda$\\
\textbf{Пример}\\
$\exp At = e^{At} = \suml_\lambda e^{t\lambda}\rho_\lambda = T \nm{diag}(e^{\lambda_1t}, \ldots, e^{\lambda_nt})T^{-1}$\\
\textbf{Свойства}
\begin{enumerate}
    ...
    \textbf{Доказательство}\\
    $(e^{At})' = (\suml_\lambda f(\lambda t)\rho_\lambda)'=\suml_\lambda \lambda e^{\lambda t}\rho_\lambda = (\suml_{\lambda} \lambda \rho_\lambda)(\suml_{\lambda} e^{t\lambda} \rho_\lambda) = Ae^{At} = e^{At}A$
\end{enumerate}
\textbf{Поиск обратной матрицы}\\
Пусть $A$ диагонализируема\\
$\fall \lambda\ \lambda \neq 0 \LRto \ex A^{-1}$\\
$A^{-1} = T\nm{diag}(\frac1{\lambda_1}, \ldots, \frac1{\lambda_n})T^{-1}$\\
$A^{-1} = \suml_\lambda \frac1\lambda \rho_\lambda$\\
\textbf{Определение}\\
...
$x = \suml_{j=1}^n x_je^j$\\
$y = \suml_{j=1}^n y_je^j$\\
Отсюда $z = \suml_{j=1}^n(x_j + iy_j) e_j$, т.е. $e_1, \ldots, e_n$ -- порождающая\\
//todo
...
\begin{enumerate}
    ...
    \textbf{Доказательство}\\
    $\mcA_\Cset e_j = \ldots = \mcA e_j = \suml_{k=1}^n a_{kj}e_k$\\
    Отсюда $A_\Cset = A$
...
$\psi(t) = t^m + a_{m-1}t^{m-1} + \ldots + a_0 = \prodl_{\lambda \text{ -- корень многочлена}} (t-\lambda)^{m(\lambda)}$, где $m(\lambda)$ -- кратность корня\\
$\psi(\mcA) = \mcA^m + a_{m-1} \mcA^{m-1} + \ldots + a_0\epsilon = \prodl_{\lambda \text{ -- корень}} (A-\lambda\epsilon)^{m(\lambda)}$\\
\textbf{Определение}\\
...
\begin{enumerate}
    ...
    $\ex! \alpha_1, \ldots, \alpha_{m-1}: \mcA^m x = \suml_{i=0}^{m-1} \alpha_i \mcA^i x$\\
    $(\mcA^m - \suml_{i=0}^{m-1} \alpha_i \mcA^i) x = \0$\\
    $\psi(t) = t^m - \suml_{i=0}^{m-1} \alpha_i t^i$ -- минимальный и определен единственным образом
\end{enumerate}
...
\begin{enumerate}
    ...
    $\fall v \in V \phi(\mcA)v = \phi(\mcA) \suml_{j=1}^m v_j e_j = \suml_{j=1}^m \phi(\mcA) v_j e_j = \suml_{j=1}^m a_j(t)\psi_j(t) v_j e_j = \0$\\
    Т.о. $\phi$ -- аннулятор
...
$\det(A-\mu E)E = \chi(\mu) E = \suml_{k=0}^n \alpha_k \mu^k E$\\
$\suml_{k=0}^n \alpha_k \mu^k E = -\mu^n B_{n-1} + \mu^{n-1}(AB_{n-1}-B_{n-2}) + \ldots + \mu (AB_1 - B_0) + AB_0$\\
Отсюда $\alpha_0 E = AB_0$\\
$\alpha_1 E = AB_1 - B_0$\\
$\vdots$\\
$\alpha_{n-1} E = AB_{n-1} - B_{n-2}$\\
$\alpha_n E = -B_{n-1}$\\
$\chi (A) = \suml_{k=0}^n \alpha_k A^k = AB_0 + A(AB_1 - B_0) + A^2 (AB^2 - B^1) + \ldots + A^{n-1} (AB_{n-1}-B_{n-2}) = \0$\\
\textbf{Следствие}\\
...
$\phi(t) = \prodl_\lambda (t-\lambda)^{m(\lambda)} = (t-\lambda)^{m(\lambda)} \prodl_{\mu \neq \lambda} (t-\mu)^{m(\lambda)} = (t-\lambda)^{m(\lambda)}\phi_\lambda(t), \phi_\lambda(t) := \prodl_{\mu \neq \lambda} (t-\mu)^{m(\lambda)}$\\
$\deg \phi = m = \suml_{\lambda} m(\lambda)$\\
\textbf{Определение}\\
...
\begin{enumerate}
    ...
    $\0 = \suml_\lambda p_\lambda(t) = \suml_\lambda a_\lambda(t)\phi_\lambda(t) = \ub{\text{делится на $(t-\lambda)^{m(\lambda)}$}}{a_\lambda(t)\ub{\text{не делится на $(t-\lambda)^{m(\lambda)}$}}{\phi_\lambda(t)}} + \ub{\text{делится на $(t-\lambda)^{m(\lambda)}$}}{\suml_{\mu \neq \lambda} a_\mu(t)\phi_\mu(t)}$\\
    ...
    \item $\oplusl_\lambda I_\lambda \subset P_{m-1}, \dim P_{m-1} = m$\\
    $\dim \oplusl_\lambda I_\lambda = \suml_\lambda m(\lambda) = m$\\
    Отсюда $P_{m-1} = \oplusl_\lambda I_\lambda$
\end{enumerate}
\textbf{Следствие}\\
$\fall p \in P_{m-1} \ex ! (p_\lambda): p_i \in I_i, p=\suml_\lambda p_\lambda$\\
В частности, для $p\equiv 1\ \ex! (p_\lambda): p_i \in I_i, 1=\suml_\lambda p_\lambda$ -- полиноминальное разложение единицы (порожденное многочленом $\phi$)\\
...
\begin{enumerate}
    ...
    $\phi(t) = \prodl_\lambda (t-\lambda)$\\
    $\deg a_\lambda(t) \leq m(\lambda) - 1 = 0$\\
    Отсюда $a_\lambda(t) = \const$
\end{enumerate}
\textbf{Теорема Лагранжа}\\
Пусть все корни $\phi(t)$ взаимно прострые\\
Т.е. $\fall \lambda: m(\lambda) = 1\ \phi(t) = \prodl_\lambda (t-\lambda)$\\
Тогда $\fall p \in P_{m-1}\ p(t) = \suml_\lambda \frac{p(\lambda)}{\phi'(\lambda)}\phi_\lambda(t)$\\
($a_\lambda = \frac{p(\lambda)}{\phi'(\lambda)}$)\\
\textbf{Доказательство}\\
$\ex! (p_\lambda): p_i \in I_i, p(t) = \suml_\lambda \ub{\phi_\lambda(t)\in I_\lambda}{a_\lambda \phi_\lambda(t)}$\\
$p(\lambda) = \suml_\mu a_\mu \phi_\mu(\lambda) = a_\lambda \phi_\lambda(\lambda) \Rto a_\lambda = \frac{p(\lambda)}{\phi_\lambda(\lambda)}$\\
$\phi(t) = \prodl_\mu (t-\mu) = (t-\lambda) \ub{\phi_\lambda(t)}{\prodl_{\mu \neq \lambda} (t-\mu)}$\\
$\phi'(t) = \suml_\mu \prodl_{\xi \neq \mu}(t-\xi)$\\
$\phi'(\lambda) = \prodl_{\xi \neq \lambda} (\lambda - \xi) = \phi_\lambda(\lambda)$\\
Отсюда $a_\lambda = \frac{p(\lambda)}{\phi'(\lambda)}$\\
\textbf{Следствие}\\
$\fall \lambda: m(\lambda) = 1$\\
$1 = \suml_\lambda p_\lambda \Rto p(t) = t = \suml_\lambda t p_\lambda$\\
\textbf{Доказательство}\\
$1 = \suml_\lambda a_\lambda \phi_\lambda(t) = \suml_\lambda \frac1{\phi'(\lambda)}\phi_\lambda(t)$\\
$t = \suml_\lambda \frac\lambda{\phi'(\lambda)}\phi_\lambda(t) = \suml_\lambda \lambda p_\lambda(t)$\\\\
Пусть $\phi(t) = \prodl_\lambda (t-\lambda)^{m(\lambda)}$ -- минимальный многочлен $\mcA \in \nm{End}(V)$\\
Построим полиноминальное разложение $1$, порождающее многочлен $\phi$\\
$1 = \suml_\lambda p_\lambda(t) = \suml_\lambda a_\lambda(t)\phi_\lambda(t)$\\
$\epsilon = \suml_\lambda \ub{\rho \text{-- спектр. проектор оператора $\mcA$}}{p_\lambda(\mcA)} = \suml_\lambda a_\lambda(\mcA) \phi_\lambda(\mcA)$\\
$\epsilon = \suml_\lambda \rho_\lambda$ -- операторное разложение единицы (порожденное оператором)\\
...
$\epsilon = \suml_\lambda p_\lambda$\\
$\mcA = \suml_\lambda \lambda p_\lambda$\\
...
\begin{enumerate}
    ...
    \item $\im \rho_\lambda = K_\lambda (\Rto \oplusl_\lambda K_\lambda = V)$
...
\begin{enumerate}
    ...
    $x = \epsilon x = \suml_\mu \rho_\mu x = \rho_\lambda x \in \im \rho_\lambda$\\
    ...
    $\fall v \in V=\oplusl_\mu K_\mu\ v = \suml_\mu \ub{\in K_\mu}{v_\mu}$ -- раскладывается единственным образом\\
    $\phi_1(\mcA) v = \suml_\mu (\mcA - \lambda \epsilon)^k \ub{\text{содержит множитель $(\mcA - \lambda \epsilon)^{m(\lambda)}$}}{\phi_\lambda(\mcA)}\ub{\in \ker (\mcA - \lambda \epsilon)^{m(\lambda)}}{v_\mu} = \suml_{\mu \neq \lambda} (\mcA - \lambda \epsilon)^k b_\mu(\mcA)(\mcA - \lambda \epsilon)^{m(\lambda)}v_\mu + (\mcA - \lambda \epsilon)^k \phi_\lambda(\mcA)v_\lambda = \0$\\
...
\textbf{Доказательство $\Rto$}\\`
$V = \oplusl_\lambda V_\lambda$\\
Пусть $\phi(t):= \prodl_{\lambda \text{ -- СЧ}} (t-\lambda)$\\
Очевидно аннулятор $\mcA$, причем минимальный\\
$\fall v \in V\ v = \suml_\lambda \ub{\in V_\lambda}{v_\lambda}$ -- раскладывается единственным образом\\
$\phi(\mcA) = \prodl_{\mu} \prodl_\lambda (\mcA - \lambda \epsilon)\ub{\in V_\mu = \ker (\mcA - \mu \epsilon)}{v_\mu} = \prodl_\mu \prodl_{\lambda \neq \mu} (\mcA - \lambda \epsilon)(\mcA - \mu \epsilon) v_\mu = \0$\\
\textbf{Доказательство $\Lto$}\\
$\fall \lambda\ m(\lambda) = 1$\\
$\phi(t) = \prodl_\lambda (t-\lambda)$\\
$\fall \lambda\ K_\lambda = \ker (\mcA - \lambda \epsilon)^1 = V_\lambda$\\
Отсюда $\oplusl_\lambda K_\lambda = V = \oplusl_{\lambda}V_\lambda \LRto \mcA$ -- о.п.с.\\
...
$\epsilon = \suml_\lambda \rho_\lambda$\\
$\mcD:= \suml_\lambda \lambda \rho_\lambda$\\
...
$\mcD v_\lambda = \suml_\mu \mu \ub{\mu \neq \lambda \Rto \ldots = 0}{\rho_\mu \ub{\in \im \rho_\lambda}{v_\lambda}} = \lambda \rho_\lambda v_\lambda = \lambda v_\lambda \Rto \lambda$ -- СЧ $\mcD$\\
$V = \oplusl_\mu \im \rho_\mu$ -- дизъюнктны\
Отсюда $\im \rho_\lambda \subset V_\lambda^\mcD$\\
$V = \oplusl_\lambda \im p_\lambda \subset \oplusl_\lambda V_\lambda^\mcD \subset V$\\
Отсюда $\im p_\lambda = V_\lambda^\mcD$\\
$\mcD$ -- о.п.с\\
$V = \oplusl_\lambda V_\lambda^\mcD$\\
$D = \suml_\lambda \rho_\lambda$ -- спектральное разложение $\mcD$\\
$\mcB := \mcA - \mcD$\\
$\nu := \max_\lambda m(\lambda)$\\
Покажем, что $\mcB^\nu = \0$\\
$(\mcA - \mcD)^\nu = (\mcA - \suml_\lambda \lambda \rho_\lambda)^\nu = (\mcA\suml_\lambda \rho_\lambda - \suml_\lambda \lambda\rho_\lambda)^\nu = (\suml_\lambda (\mcA - \lambda \epsilon)\rho_\lambda)^\lambda = \suml_\lambda (\mcA - \lambda \epsilon)^\nu\rho_\lambda = \suml_\lambda b_\lambda(\mcA) (\mcA - \lambda \epsilon)^{m(\lambda)} \phi_\lambda(\mcA) = \0$\\
$\mcB\mcD = (\mcA - \suml_\lambda \lambda \rho_\lambda)(\suml_\mu \mu \rho_\mu) = (\suml_\mu \mu \rho_\mu)(\mcA - \suml_\lambda \lambda \rho_\lambda) = \mcD\mcB$\\
...
$\mcD' = \suml_\mu \mu Q_\mu$ -- спектральное разложение\\
Достаточно доказать, что
\begin{enumerate}
    \item множество $\mu$ с.ч. $\mcD'$ совпадает с множеством с.ч. $\mcA$
    \item $\im Q_\lambda = K_\lambda(D = \suml_\lambda \lambda \rho_\lambda, \im \rho_\lambda = K_\lambda)$\\
    $\Rto \mcD = \mcD'$
    \item $\mcC = \mcA - \mcD' = \mcA - \mcD = \mcB$
\end{enumerate}
$(\mcA - \mu\epsilon)Q_\mu = (\suml_\xi \xi Q_\xi + \mcC - \mu\epsilon)Q_\mu = \mcC Q_\mu$\\
...
$\suml_\lambda Q_\lambda C Q_\mu = \suml_\mu Q_\lambda C Q_\mu$\\
...
Рассмотрим $\prodl_\mu \psi_\mu (t)$. Покажем, что это аннулятор $\mcA$\\
$\fall v \in V\ v = \suml_\mu \ub{\in \im Q_\mu}{v_\mu}$\\
$\psi(\mcA)v = \suml_\xi \psi(\mcA)v_\xi = \suml_\xi b_\xi(\mcA)\psi_\xi(\mcA)v_\xi = \0$\\
...
$V = \oplusl \im Q_\lambda \subset \oplusl_\lambda K_\lambda = V$\\
$\im Q_\lambda = K_\lambda = \im p_\lambda$\\
\textbf{Теорема 3}\\
$\mcA = \mcD + \mcB$ -- разложение Жордана\\
$\chi_\mcA \equiv \chi_\mcD$\\
\textbf{Доказательство}\\
$\mcD = \suml_\lambda \lambda p_\lambda$\\
$\mcB = \mcA - \mcD$\\
$\mcB = \0^\nu, \nu = \max m(\lambda)$\\
...
$\mcB^r x = \suml_{j=1}^m \mcB^r \ub{\in K_j = \ker \mcB^j}{x_j} + \mcB^{r+1}x' = \suml_{j=r+1}^m \mcB^r x_j +\mcB^{r+1}x' \in \suml_{j=r+1}^m \mcB^r \ol K_j + \mcB^{r+1}K$\\
Докажем дизъюнктность\\
$\suml_{j=r+1}^m \mcB^r x_j +\mcB^{r+1}x' = \0$\\
$\mcB^r\ub{\in K_r \subset Z_r \ol K_1 \oplus \ol K_r  \oplus \mcB K}{(\suml_{j=r+1}^m \ub{\in \ol K_j}{x_j} + \mcB x')}=\0$\\
$\suml_{j=r+1}^m \ub{\in \ol K_j}{x_j} + \mcB x' = \suml_{j=1}^m x_j + \mcB y'$\\
В силу единственности разложения и дизъюнктности $\ol K_j$ и $\mcB K$ $\fall j\ x_j = \0$\\
$\0 + \mcB^{r+1} = \0$ -- дизъюнктность\\\\
$K = \ol K_1 \oplusl \ldots \oplus \ol K_m \oplus \mcB K$\\
$\mcB K = \mcB \ol K_2 \oplus \mcB \ol K_3 \oplus \ldots \oplus \mcB \ol K_m \oplus \mcB^2 K$\\
$\vdots$\\
$\mcB^{m-1}K = \mcB^{m-1}\ol K_m \oplus \ub{=\0}{\mcB^m K}$\\
Отсюда следствие\\
\textbf{Следствие}\\
$K = \ol K_1 \oplusl \ldots \oplus \ol K_m \oplus \mcB \ol K_2 \oplus \ldots \oplus \mcB\ol K_m \oplus \mcB^3 \ol K_3 \oplus \ldots \oplus \mcB^3 \ol K_m \oplus \ldots \oplus \mcB^{m-1}\ol K_m$\\
...
\textbf{Следствие}\\
$\dim V_\lambda = \gamma(\lambda) = \suml_{r=1}^m d_r$\\
$\dim K_\lambda = \alpha(\lambda) = \suml_{r=1}^m r d_r$\\
...
Тогда $J_r = \oplusl_{i=1}^d \span (\mcB^{r-1} g_i, \ldots, \mcB g_i, g_i)$\\
