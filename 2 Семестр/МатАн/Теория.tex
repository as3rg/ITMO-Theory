...
Тога $f(\int_a^b \lambda(x)\phi(x)\df x) \leq \int_a^b \lambda(x)f(\phi(x))$\\
\textbf{Доказательство}\\
...
Если $p > 1$:\\
Пусть $a_i, b_i > 0$\\
Тогда $\sum_i a_i (a_i + b_i)^{p-1} \leq (\sum_i a_i^p)^{\frac1p}(\sum_i 
(a_i + b_i)^p)^{\frac1q}$\\
...
Для произвольных $a_i, b_i$ заметим, что $(\sum_i (a_i+b_i)^p)^1 \leq (\sum_i (|a_i|+|b_i|)^p)^1$\\
//todo дописать
\end{document}