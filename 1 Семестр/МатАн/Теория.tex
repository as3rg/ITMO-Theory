...
%\textbf{Замечание}
%\begin{enumerate}
%    \item $\operatorname{Int} D = \underset{G-\text{открытое}}{\bigcupl_{G:G\subset D}G}$
...
\end{enumerate}
\textbf{Определение}\\
%$G \subset \mathbb{R}, G$ - открытое множество. Тогда $G=\bigcupl_{\alpha\in A} (a_\alpha, b_\alpha)$,где $\{(a_\alpha, b_\alpha)\}_{\alpha\in A}$\\\\\\\\
...
% Пусть его центральные углы $\alpha_1, \ldots, \alpha_n, 0\leq \alpha \leq \pi, \suml_{i=1}^n \alpha_i = 2\pi$\\
