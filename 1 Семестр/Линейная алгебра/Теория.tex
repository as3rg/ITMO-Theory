...
\begin{enumerate}
    ...
    \end{enumerate}
    \item $\det A = \suml_{i=1}^n a_{ij}A_{ij}$, где $A_{ij} = (-1)^{i+j} M_{ij}$ - алгебраическое дополнение, $M_{ij}$ - минор, j фиксированный\\
    \textbf{Доказательство}\\
    $\det A = \begin{vmatrix}
        a_{11} & a_{12} & \ldots & a_{1n}\\
        0 & a_{22} & \ldots & a_{2n}\\
    ...
    \item $\suml_{i=1}^n a_{ik} A_{ij} = 0, k \neq j$\\
...
\begin{itemize}
    ...
    % Т.е. $\det A = \suml_{1 \leq j_1 < \ldots < j_{k-1} \leq n} A_{j_1\ldots j_{k-1}}^{i_1\ldots i_{k-1}}M_{j_1\ldots j_{k-1}}^{i_1\ldots i_{k-1}}$\\
    % Разложим по $k$-ой строке: $A_{j_1\ldots j_{k-1}}^{i_1\ldots i_{k-1}} = \\\suml_{j\in (1\ldots n) \setminus (j_1 \ldots j_{k-1})} (-1)^{N(i_1)+\ldots+N(i_{k-1})+N'(j_1)+\ldots+N'(j_{k-1})} a_{i_kj} M_{j_1\ldots j_{k-1} j}^{i_1\ldots i_k}$, где $N(i)/N'(j)$ - номер строки $i$/столбца $j$ исходной матрицы после вычеркиваний\\
    //todo
\end{itemize}
\textbf{Замечание}\\
